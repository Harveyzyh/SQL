...
(CASE WHEN PRICE.TM010 IS NULL THEN '是' ELSE '' END) AS 不存在单价, '' AS 价格汇总
FROM COMFORT.dbo.INVMB(NOLOCK) 
LEFT JOIN COMFORT.dbo.INVMC(NOLOCK) ON MB001 = INVMC.MC001
LEFT JOIN COMFORT.dbo.CMSMC(NOLOCK) ON CMSMC.MC001 = INVMC.MC002
...
LEFT JOIN COMFORT.dbo.INVMC(NOLOCK) ON MB001 = INVMC.MC001
LEFT JOIN COMFORT.dbo.CMSMC(NOLOCK) ON CMSMC.MC001 = INVMC.MC002
LEFT JOIN dbo.V_GetPrice AS PRICE ON PRICE.TM004 = MB001 AND MB032 = PRICE.TL004 
WHERE 1=1
-- AND INVMC.MC002 = 'P013'
AND ISNULL(INVMC.MC007, 0) < 0 
ORDER BY 材料品号
...
-- 6.清除备份表 
-- DROP TABLE COMFORT.dbo.ADMMG_BAK 
