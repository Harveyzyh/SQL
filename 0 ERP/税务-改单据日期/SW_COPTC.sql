...
-- UPDATE COPTC SET COPTC.CREATE_DATE = RTRIM(TC003)+SUBSTRING(RTRIM(A.CREATE_DATE), 9, 9), COPTC.MODI_DATE = (CASE WHEN RTRIM(A.MODI_DATE) = '' THEN A.MODI_DATE ELSE RTRIM(TC003)+SUBSTRING(RTRIM(A.MODI_DATE), 9, 9) END)

-- UPDATE COPTD SET COPTD.CREATE_DATE = RTRIM(TC003)+SUBSTRING(RTRIM(B.CREATE_DATE), 9, 9), COPTD.MODI_DATE = (CASE WHEN RTRIM(B.MODI_DATE) = '' THEN B.MODI_DATE ELSE RTRIM(TC003)+SUBSTRING(RTRIM(B.MODI_DATE), 9, 9) END)

FROM COPTC AS A
INNER JOIN COPTD AS B ON TC001 = TD001 AND TC002 = TD002
WHERE 1=1 
...
-- UPDATE COPTC SET COPTC.MODI_DATE = (CASE WHEN SUBSTRING(A.CREATE_DATE, 9, 2) = '23' THEN SUBSTRING(A.CREATE_DATE, 1, 8) + SUBSTRING(A.CREATE_DATE, 9, 2) + SUBSTRING(A.MODI_DATE, 11, 7) ELSE SUBSTRING(A.CREATE_DATE, 1, 8) + RIGHT('000' + CAST(CAST(SUBSTRING(A.CREATE_DATE, 9, 2) AS INT) + 1 AS VARCHAR(2)), 2) + SUBSTRING(A.MODI_DATE, 11, 7) END)
...
-- UPDATE COPTD SET COPTD.MODI_DATE = (CASE WHEN SUBSTRING(B.CREATE_DATE, 9, 2) = '23' THEN SUBSTRING(B.CREATE_DATE, 1, 8) + SUBSTRING(B.CREATE_DATE, 9, 2) + SUBSTRING(B.MODI_DATE, 11, 7) ELSE SUBSTRING(B.CREATE_DATE, 1, 8) + RIGHT('000' + CAST(CAST(SUBSTRING(B.CREATE_DATE, 9, 2) AS INT) + 1 AS VARCHAR(2)), 2) + SUBSTRING(B.MODI_DATE, 11, 7) END)
