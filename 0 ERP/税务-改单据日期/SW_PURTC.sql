...
-- UPDATE PURTC SET PURTC.CREATE_DATE = RTRIM(TC003)+SUBSTRING(RTRIM(A.CREATE_DATE), 9, 9), PURTC.MODI_DATE = (CASE WHEN RTRIM(A.MODI_DATE) = '' THEN A.MODI_DATE ELSE RTRIM(TC003)+SUBSTRING(RTRIM(A.MODI_DATE), 9, 9) END)

-- UPDATE PURTD SET PURTD.CREATE_DATE = RTRIM(TC003)+SUBSTRING(RTRIM(B.CREATE_DATE), 9, 9), PURTD.MODI_DATE = (CASE WHEN RTRIM(B.MODI_DATE) = '' THEN B.MODI_DATE ELSE RTRIM(TC003)+SUBSTRING(RTRIM(B.MODI_DATE), 9, 9) END)

FROM PURTC AS A
INNER JOIN PURTD AS B ON TC001 = TD001 AND TC002 = TD002
WHERE 1=1 
...
-- UPDATE PURTC SET PURTC.MODI_DATE = (CASE WHEN SUBSTRING(A.CREATE_DATE, 9, 2) = '23' THEN SUBSTRING(A.CREATE_DATE, 1, 8) + SUBSTRING(A.CREATE_DATE, 9, 2) + SUBSTRING(A.MODI_DATE, 11, 7) ELSE SUBSTRING(A.CREATE_DATE, 1, 8) + RIGHT('000' + CAST(CAST(SUBSTRING(A.CREATE_DATE, 9, 2) AS INT) + 1 AS VARCHAR(2)), 2) + SUBSTRING(A.MODI_DATE, 11, 7) END)
...
-- UPDATE PURTD SET PURTD.MODI_DATE = (CASE WHEN SUBSTRING(B.CREATE_DATE, 9, 2) = '23' THEN SUBSTRING(B.CREATE_DATE, 1, 8) + SUBSTRING(B.CREATE_DATE, 9, 2) + SUBSTRING(B.MODI_DATE, 11, 7) ELSE SUBSTRING(B.CREATE_DATE, 1, 8) + RIGHT('000' + CAST(CAST(SUBSTRING(B.CREATE_DATE, 9, 2) AS INT) + 1 AS VARCHAR(2)), 2) + SUBSTRING(B.MODI_DATE, 11, 7) END)
