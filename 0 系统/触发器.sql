...
IF UPDATE(TC009)
BEGIN
	DECLARE @IP VARCHAR(255), @MDATE VARCHAR(20),
	...
	SELECT @MDATE= (CONVERT(VARCHAR(20), GETDATE(), 112) + REPLACE(CONVERT(VARCHAR(20), GETDATE(), 24), ':', ''))
	
	IF(@TC009='V')
	BEGIN
		IF EXISTS (SELECT * FROM sysobjects WHERE name = 'Doc2VN')
		BEGIN
			INSERT INTO Doc2VN (MID, MDATE, NAME, FLAG, A001, A002, IP) VALUES(@MODIFIER, @MDATE, 'MOCTC', @TC009, @TC001, @TC002, @IP)
		END
	END
END
...
BEGIN
	...
	BEGIN
		IF EXISTS (SELECT * FROM sysobjects WHERE name = 'Doc2VN' )
		BEGIN 
			INSERT INTO Doc2VN (MID, MDATE, NAME, FLAG, A001, A002, IP) VALUES(@MODIFIER, @MDATE, 'COPTQ', @TQ006, @TQ001, @TQ002, @IP)
		END
	END
END
...
BEGIN
	...
	BEGIN
		IF (NOT EXISTS(SELECT TB002 FROM MOCTB WHERE TB001 = @TA001 AND TB002 = @TA002))
		BEGIN
			IF EXISTS (SELECT * FROM sysobjects WHERE name = 'MOCTBIsNullLog')
			BEGIN
