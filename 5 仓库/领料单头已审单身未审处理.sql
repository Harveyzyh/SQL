-- 此脚本用于将由PDA上传信息后人工审核导致的领料单错误处理
-- 需核对领料数量，产库是否与库存交易信息一致
-- 额外，当被覆盖的领料单身序号与库存交易不一致时，需手动更改领料单身序号
 
USE COMFORT

SELECT DISTINCT 
RTRIM(TA001) AS 工单别, RTRIM(TA002) AS 工单号, 
(CASE TA011 WHEN '1' THEN '未生产' WHEN '2' THEN '已发料' WHEN '3' THEN '生产中' WHEN 'Y' THEN '已完工' END) AS 工单状态, 
RTRIM(TB003) AS 品号, RTRIM(MB002) AS 品名, TB006 AS 工艺, TB009 AS 工单仓位,TB004 AS 工单需领用量, CONVERT(VARCHAR(20), (CONVERT(FLOAT, TB004) - CONVERT(FLOAT, TB005))) AS 工单未领用量, 
RTRIM(TC001) AS 领退料单别, RTRIM(TC002) AS 领退料单号, RTRIM(TC009) AS 领退料单头审核码, RTRIM(TE019) AS 领退料单身审核码, 
TE003 AS 领退料单序号, TE005 AS 领退料数量, TE027 AS 领退料库存数量, TE008 AS 领料单仓位, TE010 AS 领料单批号, 
LA011 AS 库存交易数量, (CASE WHEN LA005 = '-1' THEN '出库' WHEN LA005 = '+1' THEN '入库' END) AS 出入库, LA009 AS 交易明细仓位, LA016 AS 交易明细批号 

--UPDATE MOCTE SET TE019 = 'Y' --领料单身审核码
--UPDATE MOCTE SET TE005 = LA011, TE027 = LA011 --领退料量
--UPDATE MOCTE SET TE027 = LA011 --领退料库存数量
--UPDATE MOCTE SET TE008 = LA009 --领料单仓位
FROM MOCTA
INNER JOIN MOCTB ON TA001 = TB001 AND TA002 = TB002
LEFT JOIN MOCTE ON TB001 = TE011 AND TB002 = TE012 AND TB003 = TE004 AND TE009 = TB006
INNER JOIN MOCTC ON TE001 = TC001 AND TE002 = TC002 
LEFT JOIN INVLA ON LA006 = TC001 AND LA007 = TC002 AND LA001 = TE004 AND RTRIM(TA001) + '-' + RTRIM(TA002) = RTRIM(LA024) AND LA008 = TE003
INNER JOIN INVMB ON TB003 = MB001

WHERE 1=1
AND TA013 != 'V' AND TA011 != 'y' AND TC009 != 'V'
--AND TC009 = 'Y'
--AND TE019 = 'N'
AND TA002 = '18111236'
--AND CONVERT(FLOAT, TB004) - CONVERT(FLOAT, TB005) < 0
--AND TE005 != LA011
--AND RTRIM(TC001) + '-' + RTRIM(TC002) = '5406-2018101470'
AND TB003 = '3030317015'
--AND TE009 = '0302'
ORDER BY RTRIM(TB003), TB006, RTRIM(TC001), RTRIM(TC002), TE003


