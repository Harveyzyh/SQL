...
LEFT JOIN (SELECT TOP 1 COMPANY, CREATOR, CREATE_DATE, FLAG FROM PURTR(NOLOCK)  WHERE RTRIM(TR001)+'-'+RTRIM(TR002) = '3101-20110046') AS K ON 1=1
WHERE 1=1
AND RTRIM(TB001)+'-'+RTRIM(TB002) = '3101-20110046' 
-- AND TA007 = 'Y' 
-- AND TB025 != 'Y'
AND TR003 IS NULL
...
-- 同一个参考供应商，存在多种结束码的
SELECT TB001, TB002, TB003, TB004, TB005, TB010, TB039 FROM PURTB 
INNER JOIN 
(
	SELECT TB001 TB001C, TB002 TB002C, TB010 TB010C FROM 
	(
		SELECT DISTINCT TB001, TB002, TB010, TB039 FROM PURTB WHERE TB001 = '3101' AND TB002 = '20110066'
	) AS A 
	GROUP BY TB001 ,TB002, TB010 HAVING COUNT(*) > 1
) AS B ON TB001C = TB001 AND TB002C = TB002 AND TB010C = TB010 
WHERE 1=1 
